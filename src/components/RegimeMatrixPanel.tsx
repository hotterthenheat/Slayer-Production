  ...
  return (
    <div className="rounded-lg border border-black/60 bg-black/30 p-4 flex flex-col gap-4 shadow-inner">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xs font-black tracking-widest uppercase text-[#E5E5E5]">Market Regime — {selectedAsset?.ticker}</h2>
      ...
      <Section title="Order Clustering and Cross-Asset Signals" icon={<AlertTriangle className="w-3 h-3 text-[#FB923C]" />}>
        {edge.hawkes && <Flag label="Order-burst cascade risk (Hawkes)" value={num(edge.hawkes.cascadeProbability, 2)} active={!!edge.hawkes.ignition} tone="#FB923C" />}
      ...
      {reg.state === 'TAIL_RISK' && (
        <div className="flex items-center gap-2 text-[10px] font-bold text-[#F87171] bg-[#F87171]/10 border border-[#F87171]/40 rounded px-3 py-2">
          <AlertTriangle className="w-3.5 h-3.5" /> TAIL-RISK REGIME — unusually large moves possible; reduce position size.
        </div>
      )}
    </div>
  );
