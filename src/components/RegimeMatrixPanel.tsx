  ...
  if (!edge || !edge.regime) {
    return (
      <div className="rounded-lg border border-black/60 bg-black/40 p-4 text-center">
        <p className="text-[10px] uppercase tracking-widest text-zinc-500 animate-pulse">Computing regime matrix…</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-black/60 bg-black/30 p-4 flex flex-col gap-4 shadow-inner">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xs font-black tracking-widest uppercase text-[#E5E5E5]">Quant Regime Matrix — {selectedAsset?.ticker}</h2>
        <span className="text-[8px] text-zinc-500 uppercase tracking-widest ml-auto">11 models · live · keyless</span>
      </div>

      <Section title="Market Regime (HMM / Hurst / Ornstein-Uhlenbeck)" icon={<GitBranch className="w-3 h-3 text-[#60A5FA]" />}>
        <Flag label="Regime State" value={reg.state.replace('_', ' ')} active tone={regimeTone} />
        <Flag label="State Transition Prob" value={`${num(reg.transitionProb, 0)}%`} active={reg.transitionProb >= 60} tone={regimeTone} />
        <Flag label="Hurst — Trend Persistence" value={num(reg.hurst, 2)} active={reg.hurst > 0.55} tone="#4ADE80" />
        <Flag label="OU Reversion Half-Life" value={isFinite(ou.halfLifeBars) ? `${num(ou.halfLifeBars, 1)} bars` : '∞'} active={ou.meanReverting} tone="#60A5FA" />
        <Flag label="Mean-Reversion Drift" value={ou.meanReverting ? `θ ${num(ou.theta, 4)}` : 'none'} active={ou.meanReverting} tone="#60A5FA" />
      </Section>

      <Section title="Volatility Regimes" icon={<Waves className="w-3 h-3 text-[#C084FC]" />}>
        ...
        <Flag label="Forward Volatility Matrix" value={fwd?.detail || '—'} active={!!fwd?.active} tone="#C084FC" />
      </Section>

      <Section title="Order-Flow Microstructure (VPIN / Kyle's λ)" icon={<Gauge className="w-3 h-3 text-[#34D399]" />}>
        <Flag label="Informed Trading (VPIN)" value={num(vpin?.vpin, 3)} active={!!vpin?.toxic} tone="#F87171" />
        <Flag label="Toxic Order Flow" value={vpin?.toxic ? 'TOXIC' : 'BENIGN'} active={!!vpin?.toxic} tone="#F87171" />
        <Flag label="Kyle's λ — Price Impact" value={`${num(kyle?.impactPct, 3)}%`} active={!!kyle?.slippageRisk} tone="#FBBF24" />
        {pca && (
