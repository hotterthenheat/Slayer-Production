...
function Stat({ label, value, tone, big }: { label: string; value: React.ReactNode; tone?: string; big?: boolean }) {
  return (
    <div className="flex flex-col gap-0.5 px-3 first:pl-0">
      <span className="text-[7.5px] uppercase tracking-widest font-extrabold text-[var(--text-tertiary)] whitespace-nowrap">{label}</span>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-px bg-[var(--border)]">
        ...
        <div className="bg-[var(--surface)] p-4 flex flex-col">
          <div className="flex items-center gap-1.5 text-[9px] uppercase tracking-widest text-[var(--text-secondary)] mb-2.5">
            <Gauge className="w-3 h-3 text-[#60A5FA]" /> Rotation Scanner
          </div>
      ...
      <div className="grid grid-cols-4 sm:grid-cols-7 gap-x-4 gap-y-2 px-4 py-2.5 border-t border-[var(--border)] bg-[var(--surface-2)]">
        {components.map((c) => {
          ...
          return (
            <div key={c.key}>
              <div className="flex items-center justify-between text-[8.5px] uppercase tracking-wider text-[var(--text-secondary)] mb-1">
